...
void mmc_rescan(struct work_struct *work)
{
	...
	printk(KERN_INFO "%s %s detect a newly inserted card\n", mmc_hostname(host), __func__);
	mmc_claim_host(host);
	mmc_power_up(host);
	mmc_go_idle(host);
	 ...
	 */
	printk(KERN_INFO "%s %s search SDIO card\n", mmc_hostname(host), __func__);
	 ...
	 * ...then normal SD...
	 */
	printk(KERN_INFO "%s %s search SD card\n", mmc_hostname(host),__func__);
	 ...
	 * ...and finally MMC.
	 */
	printk(KERN_INFO "%s %s search MMC card\n", mmc_hostname(host), __func__);
...
int mmc_reinit(struct mmc_host *host)
{
	u32 ocr;
	int err = 0;
	...
	if (!err) {
		err = mmc_sd_reinit_card(host, ocr);
	}
