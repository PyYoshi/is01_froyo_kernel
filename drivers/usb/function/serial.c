...
struct gs_dev {
	...
	struct usb_cdc_header_desc gs_header_desc;
	struct usb_cdc_call_mgmt_descriptor gs_mgmt_desc;
	struct usb_cdc_acm_descriptor gs_acm_desc;
	struct usb_cdc_union_desc gs_union_desc;
...
static void gs_init_gs_header_desc(struct usb_cdc_header_desc *header_desc)
{
...
static void gs_init_usb_cdc_call_mgmt_desc(struct usb_cdc_call_mgmt_descriptor *mgmt_desc)
{
	mgmt_desc->bLength = sizeof(struct usb_cdc_call_mgmt_descriptor);
	mgmt_desc->bDescriptorType = USB_DT_CS_INTERFACE;
					/* 0x24 (USB_TYPE_CLASS | USB_DT_INTERFACE) */
	mgmt_desc->bDescriptorSubType = USB_CDC_CALL_MANAGEMENT_TYPE;
					/* 0x01 [call_mgmt_descriptor] */
	mgmt_desc->bmCapabilities = 3;  /* bits should be 00000011 (refer to 5.2.3.3) */
	mgmt_desc->bDataInterface = 1;
}

static void gs_init_usb_cdc_acm_desc(struct usb_cdc_acm_descriptor *acm_desc)
{
	acm_desc->bLength = sizeof(struct usb_cdc_acm_descriptor);
	acm_desc->bDescriptorType = USB_DT_CS_INTERFACE;
					/* 0x24 (USB_TYPE_CLASS | USB_DT_INTERFACE) */
	acm_desc->bDescriptorSubType = USB_CDC_ACM_TYPE;
					/* 0x02 [acm descriptor] */
	acm_desc->bmCapabilities = 7;   /* bits should be 00000111 */
}

static void gs_init_usb_cdc_union_desc(struct usb_cdc_union_desc *union_desc)
{
	...
	union_desc->bDescriptorSubType = USB_CDC_UNION_TYPE;
					/*  0x06 [union_desc] */
	union_desc->bMasterInterface0 = 3;
	union_desc->bSlaveInterface0 = 4;
}
...
static void gs_init_header_desc(struct gs_dev *dev)
{
	...
	dev->gs_highspeed_header[1] =
		(struct usb_descriptor_header *)&dev->gs_header_desc;
	dev->gs_highspeed_header[2] =
		(struct usb_descriptor_header *)&dev->gs_mgmt_desc;
	dev->gs_highspeed_header[3] =
		(struct usb_descriptor_header *)&dev->gs_acm_desc;
	dev->gs_highspeed_header[4] =
		(struct usb_descriptor_header *)&dev->gs_union_desc;
...
static int __init gs_module_init(void)
{
	...
	for (i = 0; i < instances; i++) {
		...
		gs_init_gs_header_desc(&gs_devices[i]->gs_header_desc);
		gs_init_usb_cdc_call_mgmt_desc(&gs_devices[i]->gs_mgmt_desc);
		gs_init_usb_cdc_acm_desc(&gs_devices[i]->gs_acm_desc);
		gs_init_usb_cdc_union_desc(&gs_devices[i]->gs_union_desc);
		...
		gs_devices[i]->gs_hs_notifyin_desc.bEndpointAddress = USB_DIR_IN;
		gs_devices[i]->gs_hs_bulkin_desc.bEndpointAddress = USB_DIR_IN;
		gs_devices[i]->gs_hs_bulkout_desc.bEndpointAddress = USB_DIR_OUT;
		gs_devices[i]->gs_fs_notifyin_desc.bEndpointAddress = USB_DIR_IN;
		gs_devices[i]->gs_fs_bulkin_desc.bEndpointAddress = USB_DIR_IN;
		gs_devices[i]->gs_fs_bulkout_desc.bEndpointAddress = USB_DIR_OUT;
...
static void gs_bind(void *_ctxt)
{
	...
	dev->gs_union_desc.bMasterInterface0 = ret;
	dev->gs_union_desc.bSlaveInterface0 = ret+1;
